# SCP-1048 (Builder Bear)

<div align="center">

![Commercial Product](https://img.shields.io/badge/Commercial-Premium_Plugin-orange?style=for-the-badge)
![C#](https://img.shields.io/badge/C%23-.NET%20Framework%204.8-512BD4?style=for-the-badge)
![EXILED](https://img.shields.io/badge/EXILED-Custom%20Plugin-green?style=for-the-badge)
![Unity](https://img.shields.io/badge/Unity-Game%20Engine-black?style=for-the-badge)
![Mirror](https://img.shields.io/badge/Mirror-Networking-orange?style=for-the-badge)
![ProjectMER](https://img.shields.io/badge/ProjectMER-Schematics-blue?style=for-the-badge)

[![Ko-fi](https://img.shields.io/badge/Ko--fi-Support%20Me-FF5E5B?style=for-the-badge\&logo=kofi\&logoColor=white)](https://ko-fi.com/jastrzabdev)

### Commercial SCP: Secret Laboratory Custom Role Showcase

Premium gameplay expansion introducing a fully playable SCP-1048 entity with custom mechanics, multiplayer synchronization, animation systems, and immersive audio interactions.

</div>

---

## Overview

SCP-1048 (Builder Bear) is a premium custom role developed for SCP: Secret Laboratory communities and multiplayer servers.

The project expands the base game with a fully playable SCP entity featuring unique gameplay mechanics, custom animations, synchronized multiplayer interactions, and a dedicated schematic designed specifically for the role.

The primary goal was to create a role that feels visually distinctive while remaining seamlessly integrated into existing SCP:SL gameplay.

---

## Video Demonstration

🎥 **Gameplay Showcase**

https://youtu.be/Df6faoootxg

---

## Project Showcase

<p align="center">
  <img src="images/1.png" alt="SCP-1048 Banner">
</p>

---

## Screenshots

### In-Game Appearance

![Screenshot](images/Misiek3.png)

### In-Game Appearance

![Screenshot](images/misiek1.png)

### In-Game Appearance

![Screenshot](images/Misiek2.png)

---

## Features

### Gameplay

* Fully playable SCP-1048 custom role
* Dedicated custom schematic
* Unique role-specific mechanics
* Custom attack handling
* Configurable gameplay balancing

### Character Systems

* Custom animation system
* Wave emote interaction
* Spatial audio integration
* Role-specific visual presentation

### Multiplayer Integration

* Multiplayer synchronization
* Server-authoritative ability execution
* Networked animation handling
* Consistent client state synchronization

### Administration

* RemoteAdmin integration
* Configurable settings
* Server-side management tools

---

## Technology Stack

### Core Technologies

* C#
* .NET Framework 4.8

### Game Integration

* EXILED Framework
* SCP: Secret Laboratory API

### Engine & Networking

* Unity
* Mirror Networking

### Content Systems

* ProjectMER Schematics

---

## Technical Highlights

* Multiplayer state synchronization
* Network-aware gameplay systems
* Custom animation controller integration
* Spatial audio implementation
* Runtime schematic management
* Event-driven gameplay architecture
* Server-authoritative ability execution
* Configurable balancing framework

---

## Development Focus

This project showcases experience with:

* C# game development
* Multiplayer systems programming
* Unity-based gameplay development
* Custom gameplay mechanics
* Network synchronization
* Event-driven architecture
* Audio system integration
* Commercial plugin development
* Production-ready server modifications

---

## Source Code Availability

Source code is not publicly available.

This repository serves as a showcase of the project's features, technologies, and development experience.

The plugin is distributed commercially.

---

## Author

### Adam Jastrzębski

[![GitHub](https://img.shields.io/badge/GitHub-MatysiakQ-181717?style=for-the-badge\&logo=github)](https://github.com/MatysiakQ)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Adam_Jastrzębski-0A66C2?style=for-the-badge\&logo=linkedin)](https://www.linkedin.com/in/adamjastrzebski/)

[![Portfolio](https://img.shields.io/badge/Portfolio-adamjastrzebski.bio-4CAF50?style=for-the-badge\&logo=googlechrome)](https://adamjastrzebski.bio/)

[![Ko-fi](https://img.shields.io/badge/Ko--fi-Support%20Me-FF5E5B?style=for-the-badge\&logo=kofi\&logoColor=white)](https://ko-fi.com/jastrzabdev)

---

### Support Development

If you enjoy my work and would like to support future projects:

☕ https://ko-fi.com/jastrzabdev

</div>
